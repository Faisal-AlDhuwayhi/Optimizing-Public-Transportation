...
class Producer:
    ...
    def create_topic(self):
        ...
        # creates the topic for this producer if it does not already exist on the Kafka Broker.
        admin_client = AdminClient({"bootstrap.servers": self.broker_properties['kafka_broker_url']})
        
        topics_metadata = admin_client.list_topics(timeout=5)
        if self.topic_name in topics_metadata.topics:
            logger.info(f"Topic ({self.topic_name}) is already exist")
            return
